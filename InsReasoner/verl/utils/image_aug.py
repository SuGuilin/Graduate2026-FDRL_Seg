...
def augment_batch(batch, config, step=0, total_steps=1):
    if "images" not in batch.non_tensor_batch:
        return batch

    from copy import deepcopy
    new_batch = deepcopy(batch)

    images_list = new_batch.non_tensor_batch["images"]  

    augmented_images_list = []
    for image_group in images_list:
        #print("clean image: ", image_group)
        augmented_group = augment_images(image_group, config, step, total_steps)
        augmented_images_list.append(augmented_group)
        #print("noisy image: ", augment_images)

    new_batch.non_tensor_batch["images"] = augmented_images_list
