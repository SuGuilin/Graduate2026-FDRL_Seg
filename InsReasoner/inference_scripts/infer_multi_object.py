...
def parse_args():
    ...
    parser.add_argument("--reasoning_model_path", type=str, default="pretrained_models/Qwen2.5-VL-3B-Instruct")
    ...
    parser.add_argument("--output_path", type=str, default="./inference_scripts/test_output_multiobject-zero.png")
...
def main():
    ...
    # plt.axis('off')  # 关闭坐标轴
    # plt.tight_layout()
    # plt.savefig('output_image_rlfusion.png', format='png')  # 保存为 PNG 格式
    # plt.close()
    # 创建 mask_overlay
    #image = np.array(image) 
    #mask_overlay = np.zeros_like(image)
    #mask_overlay[mask_all] = [255, 0, 0]  # 红色 mask
    #alpha = 0.4  # 透明度
    #combined_image = (image * (1 - alpha) + mask_overlay * alpha).astype(np.uint8)
    #output_image = PILImage.fromarray(combined_image)
    #output_image.save('output_image_1000N.png')

    # 创建一个全透明的图像
    transparent_image = PILImage.new("RGBA", image.size, (0, 0, 0, 0))
    
    # 创建掩码叠加
    mask_overlay = np.zeros((image.height, image.width, 4), dtype=np.uint8)  # RGBA格式
    mask_overlay[mask_all] = [255, 0, 0, 80]  # 半透明红色掩码
    
    # 将掩码转换为PIL图像
    mask_overlay_pil = PILImage.fromarray(mask_overlay, 'RGBA')

    # 将原始图像转换为RGBA
    image_rgba = image.convert("RGBA")
    
    # 将掩码叠加到原始图像上
    final_image = PILImage.alpha_composite(image_rgba, mask_overlay_pil)

    # 保存最终图像
    final_image.save('output_image_full_model2_bicycle_7B.png')
