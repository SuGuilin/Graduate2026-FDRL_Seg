...
def parse_args():
    ...
    parser.add_argument("--reasoning_model_path", type=str, default="/data/cr7/vlreasoner/qwen2_5vl-7b/full/sft")
    ...
    parser.add_argument("--output_path", type=str, default="./inference_scripts/test_output_multiobject_7b-sft.png")
    return parser.parse_args()


def extract_bbox_points_think(output_text, x_factor, y_factor):
    # json_match = re.search(r'\s*(.*?)\s*</answer>', output_text, re.DOTALL)
    output_text = re.sub(r'(\d) +(\d)', r'\1, \2', output_text)
    print(output_text)
    json_match = True
    if json_match:
...
def main():
    ...
    message = [{
        "role": "system",
        "content": "You are a assistant. For each question, output a structured answer in JSON format.",
        "role": "user",
        "content": [
        {
            "type": "image", 
