...
def get_image_paths(dataroot, txt="train.txt"):
    paths = None  # return None if dataroot is None
    partition_txt = os.path.join(os.path.dirname(dataroot), txt)

    png_id = read_lines_from_txt(partition_txt)
    paths = []
    for png in png_id:
        paths.append(dataroot + "/" + png)
    ...
    #        paths.append(dataroot + png_id[i])
    return paths
...
def read_lines_from_txt(txt_file):
    try:
        ...
        print(f"ERROR: Error reading file {txt_file}: {e}")
        return []
...
def get_text_paths(dataroot, txt="train.txt"):
    paths = None  # return None if dataroot is None
    paths = None  # return None if dataroot is None
    partition_txt = os.path.join(os.path.dirname(dataroot), txt)
    png_id = read_lines_from_txt(partition_txt)
    paths = []
    for png in png_id:
        paths.append(dataroot + "/" + os.path.splitext(png)[0] + ".txt")
    #if isinstance(dataroot, str):
    #    paths = sorted(_get_paths_from_texts(dataroot))
    #elif isinstance(dataroot, list):
    #    paths = []
    #    for i in dataroot:
    #        paths += sorted(_get_paths_from_texts(i))
    return paths

def _get_paths_from_texts(path):
    """从文本目录获取所有文本文件路径"""
    assert os.path.isdir(path), '{:s} is not a valid directory'.format(path)
    texts = []
    for dirpath, _, fnames in sorted(os.walk(path)):
...
def is_text_file(filename):
    """检查文件是否为文本文件"""
    TEXT_EXTENSIONS = ['.txt', '.text', '.md', '.csv', '.json', '.xml', '.yaml', '.yml']
    return any(filename.lower().endswith(extension) for extension in TEXT_EXTENSIONS)
...
def imread_uint(path, n_channels=1):
    ...
    if n_channels == 1:
        ...
        #new_height, new_width = (height // 4) * 4, (width // 4) * 4
        ...
        return img, img, height, width #y_channel, uv_channels  # Return both Y and UV channels
