...
import os
import sys
sys.path.append('/home/sgl/zigma/dis_mamba/mamba_ssm')
import warnings
...
try:
    ...
    mamba_inner_fn_no_out_proj = mamba_inner_fn
except ImportError:
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, input_h=None, input_w=None, extra_emb=None, inference_params=None):
        """
        hidden_states: (B, L, D)
        extra_emb: (B, L, D)
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v0":
                ...
                xz_b = rearrange(xz, "b d (h w) -> b d h w", h=input_h, w=input_w)
                ...
                #print("mamba v6:", out_c.shape)
                out = rearrange(out, "b l d -> b d l")
                out_b = rearrange(out_b, "b l d -> b d l")
                out_c = rearrange(out_c, "b (w h) d -> b d w h", w=input_w, h=input_h)
                out_c = rearrange(out_c.transpose(-2, -1).contiguous(),
                                  "b d h w -> b d (h w)",
                                  h=input_h,
                                  w=input_w)
                out_d = rearrange(out_d.flip([-1]), "b (w h) d -> b d w h", w=input_w, h=input_h)
                ...
                if not self.if_devide_out:
                    ...
                    if self.use_norm:
                        #print("v6 out:", out.shape)
                        #print("v6 out_b:", out_b.shape)
                        #print("v6 out_c:", out_c.shape)
                        #print("v6 out_d:", out_d.shape)
